...
class Main extends CI_Controller {
    ...
    function index() {
        ...
        if(empty($this->decrypt_profilid)) {
    ...
    function lupapassword() {
        //title
        $data['title'] = "Lupa Password | Primaniyarta ".date("Y");
    ...
    function kirimlupapassword() {
        ...
        //cek email terdaftar
        $cek = $this->primamod->cekemailuser($email);
        if($cek==TRUE) {
            $this->primamod->resetpassword($email);
        }
        else {
    ...
    //halaman permintaan reset password terkirim
    function resetpasswordterkirim() {
        //title
        $data['title'] = "Reset Password | Primaniyarta ".date("Y");
        ...
        //view
        $this->load->view('tpl_header', $data);
        $this->load->view('resetpasswordterkirim', $data);
        $this->load->view('tpl_footer', $data);
    }
    ...
    function resetpassword() {
        ...
        //cek apakah ada data iduser
        if(empty($iduser)) {
            redirect('main/resetpasswordgagal');
        }
        
        //ambil data user
        $data['user'] = $this->primamod->ambildatauser($decrypt_id, '')->row();
    ...
    function gantipassword() {
        ...
        $this->primamod->gantipassword($data);
    }
    
    function resetpasswordberhasil() {
        //title
        $data['title'] = "Reset Password | Primaniyarta ".date("Y");
        ...
        //view
        $this->load->view('tpl_header', $data);
        $this->load->view('resetpasswordberhasil', $data);
        $this->load->view('tpl_footer', $data);
    }
    
    function resetpasswordgagal() {
        //title
        $data['title'] = "Reset Password | Primaniyarta ".date("Y");
        ...
        //view
        $this->load->view('tpl_header', $data);
        $this->load->view('resetpasswordgagal', $data);
        $this->load->view('tpl_footer', $data);
    }
    ...
    function daftar() {
        //cek status login
        if(!empty($this->session->sessuserid)) {
    ...
    /*** HALAMAN AKUN PRIMANIYARTA START ***/
    ...
    function tambahprofil() {
        ...
        //ambil kategori
        $data['kat'] = $this->primamod->ambilkategori("", "9");
    ...
    function pilihkategori() {
        ...
        $data['katp'] = $this->primamod->ambilkategori('', '9');
    ...
    function produk() {
        ...
        $data['idsurvey'] = $bs==TRUE ? $s->idsurvey : "";
    ...
    function lainlain() {
        ...
        //variable boolean
        $bp = FALSE; $bs = FALSE; $ba = FALSE; $bc = FALSE; $bl = FALSE;
        ...
        //idsertifikat
        $idsertifikat = $this->secure->decrypt_url($this->uri->segment(4));
        if(!empty($idsertifikat)) {
            $s = $this->primamod->ambilsertifikat($idsertifikat, "")->row();
            $ns = $this->primamod->ambilsertifikat($idsertifikat, "")->num_rows();
            if($ns>0) { $bs = TRUE; }
        }

        //idpenghargaan
        $idpenghargaan = $this->secure->decrypt_url($this->uri->segment(4));
        if(!empty($idpenghargaan)) {
            $a = $this->primamod->ambilpenghargaan($idpenghargaan, "")->row();
            $na = $this->primamod->ambilpenghargaan($idpenghargaan, "")->num_rows();
            if($na>0) { $ba = TRUE; }
        }

        //idcsr
        $idcsr = $this->secure->decrypt_url($this->uri->segment(4));
        if(!empty($idcsr)) {
            $c = $this->primamod->ambilcsr($idcsr, "")->row();
            $nc = $this->primamod->ambilcsr($idcsr, "")->num_rows();
            if($nc>0) { $bc = TRUE; }
        }

        //idlh
        $idlh = $this->secure->decrypt_url($this->uri->segment(4));
        if(!empty($idlh)) {
            $l = $this->primamod->ambillh($idlh, "")->row();
            $nl = $this->primamod->ambillh($idlh, "")->num_rows();
            if($nl>0) { $bl = TRUE; }
        }
        ...
        $data['sertifikat'] = $this->primamod->ambilsertifikat("", $idprofil);
        //ambil data penghargaan untuk ditampilkan ke tabel
        $data['penghargaan'] = $this->primamod->ambilpenghargaan("", $idprofil);
        //ambil data csr untuk ditampilkan ke tabel
        $data['csr'] = $this->primamod->ambilcsr("", $idprofil);
        //ambil data lhk untuk ditampilkan ke tabel
        $data['lh'] = $this->primamod->ambillh("", $idprofil);
        ...
        $data['idsertifikat'] = $idsertifikat;
        $data['vtahunsertifikat'] = $bs==TRUE ? $s->tahun : "";
        $data['vnmsertifikat'] = $bs==TRUE ? $s->nmsertifikat : "";
        $data['vdikeluarkan'] = $bs==TRUE ? $s->dikeluarkan_oleh : "";

        $data['idpenghargaan'] = $idpenghargaan;
        $data['vtahunpenghargaan'] = $ba==TRUE ? $a->tahun : "";
        $data['vnmpenghargaan'] = $ba==TRUE ? $a->nmpenghargaan : "";
        $data['vdiberikan'] = $ba==TRUE ? $a->diberikan_oleh : "";

        $data['idcsr'] = $idcsr;
        $data['vtahuncsr'] = $bc==TRUE ? $c->tahun : "";
        $data['vkegiatan'] = $bc==TRUE ? $c->kegiatan : "";
        $data['vstakeholder'] = $bc==TRUE ? $c->stakeholder : "";;

        $data['idlh'] = $idlh;
        $data['vtahunlh'] = $bl==TRUE ? $l->tahun : "";
        $data['vkegiatanlh'] = $bl==TRUE ? $l->kegiatan : "";
        $data['vstakeholderlh'] = $bl==TRUE ? $l->stakeholder : "";
    ...
    //simpan sertifikasi
    function simpansertifikasi() {
        //returns all POST items with XSS filter
        $data = $this->input->post(NULL, TRUE); 

        $this->primamod->simpansertifikasi($data);
    }

    //hapus sertifikasi
    function hapussertifikasi() {
        //returns all POST items with XSS filter
        $idsertifikat = $this->input->post('idsertifikat'); 

        $this->primamod->hapussertifikasi($idsertifikat);
    }
    ...
    function simpanpenghargaan() {
        ...
        $this->primamod->simpanpenghargaan($data);
    }
    ...
    function hapuspenghargaan() {
        ...
        $this->primamod->hapuspenghargaan($idpenghargaan);
    }
    ...
    function simpancsr() {
        ...
        $this->primamod->simpancsr($data);
    }

    //hapus csr
    function hapuscsr() {
        //returns all POST items with XSS filter
        $idcsr = $this->input->post('idcsr'); 

        $this->primamod->hapuscsr($idcsr);
    }
    ...
    function dokumen() {
        ...
        //title
        $data['title'] = "Dokumen Pendukung | Primaniyarta ".date("Y");
        ...
        $idprofil = $this->decrypt_profilid;
        //iddokumen
        $iddok = $this->secure->decrypt_url($this->uri->segment(3));
        
        //ambil data dokumen untuk ditampilkan ke tabel
        $data['dok'] = $this->primamod->ambildokumen($idprofil, "");
        ...
        if(!empty($iddok)) {
            $p = $this->primamod->ambildokumen($idprofil, $iddok)->row();
            $np = $this->primamod->ambildokumen($idprofil, $iddok)->num_rows();
            if($np>0) { $bp = TRUE; }
        }
        ...
        $this->load->view('akun/tpl_header', $data);
        $this->load->view('akun/tpl_sidemenu', $data);
        $this->load->view('akun/eksdokumen', $data);
    }
    ...
    function simpandokumen() {
        ...
        $config['allowed_types'] = 'pdf|jpg|jpeg|xls|xlsx'; 
        ...
        if(empty($linkdownload)) {
            if($status=="tambah") {
                if($this->upload->do_upload("file")) {
                    ...
                    $this->primamod->simpandokumen($data);     
                }
                else {
                    echo "failed upload <br><br><a href='".base_url('main/dokumen')."'>Kembali ke halaman Dokumen Pendukung</a>"; 
                }
            }
            else {
                if(empty($_FILES['file']['name'])) {
                    $this->primaamod->simpandokumen($data); 
                }
                else {
                    if($this->upload->do_upload("file")) {
                        // Get data about the file
    ...
    //hapus data dokumen pendukung 
    function hapusdokumen() {
        $idprofil = $this->input->post('idprofil');
        $iddok = $this->input->post('iddok');
        
        $this->primamod->hapusdokbuyer($idprofil, $iddok);
    }
    
    /*** AKUN PRIMANIYARTA END ***/
    ...
    function userprofil() {
        ...
        //title
        $data['title'] = "Profil User | Primaniyarta ".date("Y");
        
        //nama user
        $data['nmuser'] = $this->nmuser;
        
        //ambil data profil user
        $data['user'] = $this->primamod->ambildatauser($this->decrypt_userid, $this->decrypt_email);

        //ambil data profil user
        $iduser = $this->decrypt_userid;
        $email = $this->decrypt_email;
        $p = $this->primamod->ambildatauser($iduser, $email)->row();
    ...
    function updateprofiluser() {
        ...
        $this->primamod->updateprofiluser($data);
    }
    
    function userpassword() {
        //title
        $data['title'] = "Ganti Password | Primaniyarta ".date("Y");
    ...
    function kepaladaerah() {
        ...
        $idprofil = $this->decrypt_profilid;
        if(!empty($idprofil)) {
            $p = $this->primamod->ambilkepaladaerah($idprofil)->row();
            $np = $this->primamod->ambilkepaladaerah($idprofil)->num_rows();
            if($np>0) { $bp = TRUE; }
        }
    ...
    function simpanprofilgub() {
        //returns all POST items with XSS filter
        $data = $this->input->post(NULL, TRUE); 

        $this->primamod->simpanprofilgub($data);
    }
    ...
    function kebijakan() {
        ...
        $data['kebijakan'] = $this->primamod->ambilkebijakangub("", $this->decrypt_profilid);

        //variable boolean
        $bp = FALSE;

        //cek data idprofil di tabel pma_users
        $idkebijakan = $this->secure->decrypt_url($this->uri->segment(3));
        if(!empty($idkebijakan)) {
            $p = $this->primamod->ambilkebijakangub($idkebijakan, $this->decrypt_profilid)->row();
            $np = $this->primamod->ambilkebijakangub($idkebijakan, $this->decrypt_profilid)->num_rows();
            if($np>0) { $bp = TRUE; }
        }
    ...
    function simpankebijakangub() {
        ...
        if($this->upload->do_upload("file")) {
            ...
            $this->primamod->simpankebijakangub($data);     
        }
        else {
            echo 'failed '.$this->input->post('file'); 
        }
    ...
    function hapuskebijakan() {
        ...
        $h = $this->primamod->kepaladaerahkebijakanhapus($idkebijakan);
        if($h==TRUE) {
    ...
    function kisahkepaladaerah() {
        ...
        $idprofil = $this->decrypt_profilid;
        if(!empty($idprofil)) {
            $p = $this->primamod->ambilkisahgub($idprofil)->row();
            $np = $this->primamod->ambilkisahgub($idprofil)->num_rows();
            if($np>0) { $bp = TRUE; }
        }
    ...
    function simpankisahgub() {
        //returns all POST items with XSS filter
        $data = $this->input->post(NULL, TRUE); 

        $this->primamod->simpankisahgub($data);
    }

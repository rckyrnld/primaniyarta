...
class Primamod extends CI_Model {
    ...
    function simpanprofil($data) {
        ...
        if(empty($data['idprofil'])) {
            ...
            $this->db->where('email',  $this->secure->decrypt_url($this->session->sessemail));
    ...
    //simpan profil eksportir
    function simpan_profil_gub($data) {
        $decrypt_userid = $this->secure->decrypt_url($this->session->sessuserid);
        $data1 = array(
        ...
        if($this->db->affected_rows()>0) {
            //redirect ke halaman pendaftaran sukses
            redirect('akun/kepaladaerah/profil');
        }
        else {
            echo "ERROR";
        }
    ...
    function simpan_kebijakan_gub($data) {
        $decrypt_userid = $this->secure->decrypt_url($this->session->sessuserid);
        $data1 = array(
            'idprofil' => $data['idprofil'],
    ...
    function kepala_daerah_kebijakan_hapus($idkebijakan) {
    ...
    function simpan_kisah_gub($data) {
        $data1 = array(
        ...
        if(empty($data['idprofil'])) {
            ...
            $this->db->where('idprofil', $data['idprofil']);
            $this->db->update('pma_gub_kisah', $data1);
        }
         ...
         if($this->db->affected_rows()>0) {
             //redirect ke halaman pendaftaran sukses
             redirect('akun/kepaladaerah/kisah');
         }
         else {
             echo "ERROR";
         }
    ...
    function ambil_kebijakan_gub($idkebijakan, $idprofil) {
        $this->db->select('*');
        $this->db->from('pma_gub_kebijakan');
    ...
    function ambil_kisah_gub($idprofil) {
        $this->db->select('*');
        $this->db->from('pma_gub_kisah');
        $this->db->where('idprofil', $idprofil);
        $q = $this->db->get();
    ...
    function cekkepaladaerah($idprofil) {
        $this->db->select('*');
        $this->db->from('pma_gub_profil');
        $this->db->join('pma_users', 'pma_users.idprofil=pma_gub_profil.idprofil');
        $this->db->where('pma_gub_profil.idprofil', $idprofil);
